...
for i = 1
    for j = 1:n
        i,j
        [totalmethane, tankmass, total_heat, wmin, winitial, totalexhauststeam, burnerheat, time, cells] = SOFCdriver(Temps(j), 0.98, 1, 1);
...
% % end
% 
[LNG_min, Temp_LNG] = min(methane);  Temp_minFuel = Temps(Temp_LNG);
[Water_min, Temp_water] = min(water);  Temp_minH2O = Temps(Temp_water);
[Steam_min, Temp_steam] = min(steam);  Temp_minSteam = Temps(Temp_steam);
